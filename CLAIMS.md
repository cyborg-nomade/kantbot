...
| <a id="k-028"></a>K-028 | A supported sensible license must be replayable against the actual selected intuitions; declarations of success alone cannot certify object formation or applicability. | Current | [ADR 0007](docs/decisions/0007-replayable-sensible-licenses.md); [sensible procedures](SENSIBLE_PROCEDURES.md) | The accepted implementation binds conditions to procedures and recomputes evidence. Phase 3 revisits the bounded procedure vocabulary without relaxing replayable licensing. |
| <a id="k-029"></a>K-029 | The initial strip sensor supplies anonymous momentary patches without supplying persistent world identity; hidden marker labels and unadmitted future snapshots are not receptive grounds. | Provisional | [Proposed ADR 0008](docs/decisions/0008-deterministic-strip-sensor.md); [toy-world boundary](TOY_WORLD.md) | Pending maintainer review: use frame-local sample IDs and a separate evaluator export; test hidden-state invariance. Phase 3 must synthesize identity, and later perceptual work revisits supplied segmentation. |
