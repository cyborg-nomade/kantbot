...
| <a id="k-029"></a>K-029 | The initial strip sensor supplies anonymous momentary patches without supplying persistent world identity; hidden marker labels and unadmitted future snapshots are not receptive grounds. | Current | [ADR 0008](docs/decisions/0008-deterministic-strip-sensor.md); [toy-world boundary](TOY_WORLD.md) | The accepted sensor uses frame-local sample IDs and a separate evaluator export; tests enforce hidden-state invariance. Phase 3 must synthesize identity, and later perceptual work revisits supplied segmentation. |
